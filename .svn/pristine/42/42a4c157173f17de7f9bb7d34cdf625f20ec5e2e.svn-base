    ...
    <script>
        $(document).ready(function () {
            ...
            $("input[name='pay_name']").blur(function () {
                ...
                if (pay_name == "") {
                    $("#check").text('请输入您的真实姓名').css('display', 'block');
                    return;
                } else {
                    $("#check").css('display', 'none');
                }
            });
            $("input[name='pay_account']").blur(function () {
                var pay_account = $("input[name='pay_account']").val();
                var phone = pay_account.match(/^(0|86|17951)?(13[0-9]|15[012356789]|17[0678]|18[0-9]|14[57])[0-9]{8}$/);
                var email = pay_account.match(/^([a-zA-Z0-9_-])+@([a-zA-Z0-9_-])+(.[a-zA-Z0-9_-])+/);
                if (phone == null && email == null) {
                    $("#check").text('您的账户不合法，请输入正确的信息').css('display', 'block');
                    $("input[name='pay_account']").val('');
                    return;
                } else {
                    $("#check").css('display', 'none');
                }
            ...
            $("input[name='pay_account_cfm']").blur(function () {
                var pay_account = $("input[name='pay_account']").val();
                var pay_account_cfm = $("input[name='pay_account_cfm']").val();
                if (pay_account != pay_account_cfm) {
                    $("#check").text('两次账户信息不一致，请重新输入').css('display', 'block');
                    return;
                } else {
                    $("#check").css('display', 'none');
                }
            ...
            $("#send").click(function () {
                ...
                var data = {};
                data.name = $("input[name='pay_name']").val();
                data.account = $("input[name='pay_account']").val();
                data.money = $(".glyphicon").text();
                if (data.name == "") {
                    $("#check").text('对不起，您的姓名不能为空').css('display', 'block');
                    return;
                }
                if (data.account == "") {
                    $("#check").text('对不起，您的账户不能为空').css('display', 'block');
                    return;
                }
                ...
                $.ajax({
                    url: '/welcome/post_pay',// 跳转到 action
                    data: data,
                    type: 'post',
                    dataType: 'json',
        ...
        <div class="panel-body clear">
            ...
            <?php if(empty($name)):?>
                <div class="form-group">
                    <label for="">姓名</label>
                    <input type="text" name="pay_name" class="form-control" placeholder="您的支付宝真实姓名">
                </div>
            <?php else:?>
                <div class="form-group">
                    <label for="">姓名</label>
                    <input type="text" name="pay_name" class="form-control" placeholder="您的支付宝真实姓名" value="<?=$name?>">
                </div>
            <?php endif;?>
            ...
            <?php if(empty($account)):?>
                <div class="form-group">
                    ...
                    <input type="text" name="pay_account" class="form-control" placeholder="您的支付宝账号">
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail2">确认您的帐号</label>
                    <input type="text" name="pay_account_cfm" class="form-control" placeholder="再次输入您的支付宝账号">
                </div>
            <?php else:?>
                <div class="form-group">
                    <label for="exampleInputEmail2">账号</label>
                    <input type="text" name="pay_account" class="form-control" placeholder="您的支付宝账号" value="<?=$account?>">
                </div>
                <div class="form-group">
                    <label for="exampleInputEmail2">确认您的帐号</label>
                    <input type="text" name="pay_account_cfm" class="form-control" placeholder="再次输入您的支付宝账号" value="<?=$account?>" >
                </div>
            <?php endif;?>
